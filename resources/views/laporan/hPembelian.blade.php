...
<head>
    <meta charset="UTF-8">
    <title>Laporan Pembelian {{ \Carbon\Carbon::parse($tanggalM)->format('d M Y') }} - {{ \Carbon\Carbon::parse($tanggalD)->format('d M Y') }}</title>
    <style>
        body {
        ...
        <tbody>
            @forelse($pembelian as $index => $item)
                <tr>
                    ...
                    <td></td>
                    @endif
                    <td class="text-center">{{ $item->no_trans ?? 'No Transaksi Tidak Ada' }}</td>
                    <td class="text-center">{{ $item->supplier->nama ?? 'Umum'}}</td>
                    @if($detail)
                    @foreach ($item->DetailPembelian as $details )
                    <tr>
                        <td></td>
                        <td class="text-center">{{ $details->produk->nama ?? 'Tidak Ada'}} {{ $details->varian->nama_varian ?? '' }}</td>
                        <td class="text-center">{{ $details->jumlah }}</td>
                        <td class="text-end">Rp {{ number_format($details->harga, 0, ',', '.') }}</td>
                        <td class="text-end">Rp {{ number_format($details->total, 0, ',', '.') }}</td>
                    </tr>
                        @endforeach
                        <tr>
                            <th class="text-center" colspan="4">Subtotal</th>
