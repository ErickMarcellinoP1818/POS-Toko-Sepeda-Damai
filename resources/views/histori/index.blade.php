...
<html lang="en">
<head>
    <title>Program Toko - Penjualan</title>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            ...
                            <button type="button" class="btn btn-primary mb-3" onclick="showoption()">
                                <i class="bi bi-printer"></i> Print Detail Penjualan
                            </button>
                        </div>
                        <div class="card-body">
                            <div>
                                <H2>Total Penjualan: Rp. {{ number_format($total, 0, ',', '.') }}</H2>
                            </div>
                                    ...
                                    <tbody>
                                        @forelse ($nota as $index => $item)
                                            <tr class="nota-row" data-index="{{ $index }}">
                                                ...
                                                <td class="text-center">{{ $item->kasir->name ?? 'Kasir Tidak Ditemukan' }}</td>
                                                <td class="text-center">{{ $item->metode }}</td>
                                                <td class="text-center">Rp {{ number_format($item->total, 0, ',', '.') }}</td>
                                                <td class="text-center">{{ $item->status }}</td>
                                                <td class="text-center">
                                                   <button class="btn btn-info btn-sm" onclick="showNotaDetail({{ $item->id }})">Detil Nota </button>
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
